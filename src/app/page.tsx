...
// Updated Subject Type with dates
type Subject = {
  id: number;
  name: string;
  code: string;
...
type QueueItem = {
  ...
  subjectCode: string;
  paper: string;
  examDate: string; // NEW: Holds the specific date for this paper
  count: number;
};
...
export default function Home() {
  ...
  // Updated new subject state to include dates
  ...
  // --- HELPER: GET DATE FOR PAPER ---
  const getDateForPaper = (subject: Subject, paperKey: "p1" | "p2" | "p3") => {
    if (paperKey === "p1") return subject.date_p1;
    if (paperKey === "p2") return subject.date_p2;
    if (paperKey === "p3") return subject.date_p3;
    return "";
  };
  ...
  const createQueueItem = (
    ...
    paper: paperName,
    examDate: date || "___/___/2026", // Default if empty
    count: count,
  });
  ...
  const handleBatchAdd = (paperType: string) => {
    ...
    const batchItems = subjects.map((sub) => {
      let date = "";
      if (paperType === "Paper 1") date = sub.date_p1;
      if (paperType === "Paper 2") date = sub.date_p2;
      if (paperType === "Paper 3") date = sub.date_p3;

      return createQueueItem(paperType, sub, 0, date);
    });
      ...
      .insert([
        {
          ...
          date_p1: newSubject.date_p1,
          date_p2: newSubject.date_p2,
          date_p3: newSubject.date_p3,
        },
      ])
      .select();
        ...
        pagesHtml += `
            <div class="page page-break">
              ...
              <div class="row" style="margin-top:30px;">
                ...
                <div class="field">${item.examDate}</div>
              </div>
      ...
      {isSubjectModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-2xl shadow-2xl w-96">
            ...
            {/* DATE FIELDS FOR NEW SUBJECT */}
            <input
              className="w-full mb-2 p-3 bg-gray-50 border rounded-xl text-sm"
              placeholder="Date Paper 1 (DD/MM/YYYY)"
              value={newSubject.date_p1}
              onChange={(e) =>
                setNewSubject({ ...newSubject, date_p1: e.target.value })
              }
            />
            <input
              className="w-full mb-2 p-3 bg-gray-50 border rounded-xl text-sm"
              placeholder="Date Paper 2"
              value={newSubject.date_p2}
              onChange={(e) =>
                setNewSubject({ ...newSubject, date_p2: e.target.value })
              }
            />
            {selectedExam?.max_papers === 3 && (
              <input
                className="w-full mb-4 p-3 bg-gray-50 border rounded-xl text-sm"
                placeholder="Date Paper 3"
                value={newSubject.date_p3}
                onChange={(e) =>
                  setNewSubject({ ...newSubject, date_p3: e.target.value })
                }
              />
            )}
